...
def __init__(self, input_shape, latent_shape, output_shape,
    ...
    self.feat_extractor = self.get_Extractor(input_shape)
    self.feat_analyzer = self.get_Analyzer(latent_shape)
    self.total_loss = tf.keras.metrics.Mean(name='total_loss')
    self.data_loss = tf.keras.metrics.Mean(name='data_loss')
    self.physics_loss = tf.keras.metrics.Mean(
        name='physics_loss')
    self.phi_loss = tf.keras.metrics.Mean(name='phi_loss')
    self.fn_loss = tf.keras.metrics.Mean(name='fn_loss')

@property
def metrics(self):
    return [self.total_loss, self.data_loss,
            self.physics_loss, self.phi_loss,
            self.fn_loss]
...
def get_Extractor(self, input_shape):
    ...
    return Model(inputs=x_input,
                 outputs=[z_phi, z_freqs, z],
                 name='feat_extractor')

def get_Analyzer(self, latent_shape):
    alpha = 0.01  # -> To match Lin, et al (2017)
    # --- 6th Layer ---
    z = tf.keras.Input(shape=latent_shape[1:])
    h = Dense(256)(z)
    h = LeakyReLU(alpha=alpha)(h)
    # --- 7th Layer ---
    h = Dense(128)(h)
...
def fit_physics(self, X_train, y_train, phi_train, fn_train,
                epochs=1, batch_size=10, verbose=1):
    # Convert all data into single dataset as Dataset dtype
    dataset = tf.data.Dataset.from_tensor_slices(
        (X_train, y_train, phi_train, fn_train)
        )
    # Set batch size of dataset
    dataset = dataset.batch(batch_size)
    ...
    for epoch in range(epochs):
        ...
        # Perform training for each batch of data per epoch
        # Returns a single batch per loop iteration
        for x, y, phi, fn in dataset:
            train_metrics = self.train_step(x, y, phi, fn)
        end_time = time.time()
        T = end_time - start_time
        if (verbose == 1) or (verbose == 2):
            logs = ""
...
def train_step(self, X, y, phi=None, fn=None,
               alpha1=1.0, alpha2=1.0):
    # Define loss objects
    MSE_loss_object = tf.keras.losses.MeanSquaredError()
    with tf.GradientTape() as tape:
        # Extract features in latent space
        z_phi, z_fn, z = self.feat_extractor(X)
        # Predict structural health from latent space
        y_pred = self.feat_analyzer(z)
        # --- Calculate losses ---
        # Data Loss
        data_loss = MSE_loss_object(y, y_pred)
        if (phi is not None) & (fn is not None):
            # Physics Loss
            phi_loss = MSE_loss_object(z_phi, phi)
            fn_loss = MSE_loss_object(z_fn, fn)
            physics_loss = tf.cast(phi_loss + fn_loss,
                                   tf.float32)
            # Total Loss
            total_loss = alpha1 * data_loss 
                       + alpha2 * physics_loss  
        else:
            total_loss = alpha1 * data_loss
    ...
    if (phi is not None) & (fn is not None):
        ...
        self.phi_loss.update_state(phi_loss)
        self.fn_loss.update_state(fn_loss)
        self.physics_loss.update_state(physics_loss)
    self.total_loss.update_state(total_loss)
    return {m.name: m.result().numpy() for m in self.metrics}

def predict(self, X):
    _, _, z_pred = self.feat_extractor.predict(X)
    y_pred = self.feat_analyzer.predict(z_pred)
    return [y_pred, z_pred]

def freeze_layers(self):
    for ii in range(len(self.feat_extractor.layers)):
        self.feat_extractor.layers[ii].trainable = False
    return

def unfreeze_layers(self):
    for ii in range(len(self.feat_extractor.layers)):
        self.feat_extractor.layers[ii].trainable = True
    return
...
def get_features(self, X):
    z_phi, z_freqs, _ = self.feat_extractor.predict(X)
    return [z_phi, z_freqs]
...
def save_model(self, modelName, history=None,
    ...
    # Model A
    self.feat_extractor.save_weights(
        os.path.join(
            save_path,
            "Weights_" + modelName + "_Model A.h5"
            )
        )
    # Model B
    self.feat_analyzer.save_weights(
        os.path.join(
            save_path,
            "Weights_" + modelName + "_Model B.h5"
            )
        )

def load_trained_weights(self, fileA, fileB):
    # --- Load weights .h5 files of each sub model ---
    # Model A
    self.feat_extractor.load_weights(fileA)
    # Model B
    self.feat_analyzer.load_weights(fileB)

def load_history(self, path, save_key):
    history = loadmat(
        os.path.join(
            path,
            ('History_PGML4SHM_' + save_key + '.mat')
            )
        )
